...
TMP_DB_PATH = os.path.join(os.path.dirname(__file__), "tmp_data.json")
_MEMORY_DB = None
...
def login():
    ...
    for u in db:
        if (u["email"].lower() == login_id or u.get("phone") == login_id) and u[
            "password"
        ] == data.get("password"):
            return (
                jsonify({"user": {k: v for k, v in u.items() if k != "password"}}),
                200,
            )
...
def update_profile():
    ...
    for idx, u in enumerate(db):
        if u["email"].lower() == old_email:
            ...
            return (
                jsonify(
                    {"user": {k: v for k, v in db[idx].items() if k != "password"}}
                ),
                200,
            )
